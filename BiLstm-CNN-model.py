...
"""
Bi-LSTM & double CNN & double linear
"""
...
def convertNetOutput(ratingOutput, categoryOutput):
    ...
    """
    # m = tnn.LogSoftmax()
    # ratingOutput = m(ratingOutput)
    # categoryOutput = m(categoryOutput)
...
class network(tnn.Module):
    ...
    def __init__(self):
        ...
        self.filter1_num = 75
        self.window_sizes = [1,3,5]
        self.window_num = 3
        self.cnn_layers = tnn.ModuleList([
            tnn.Conv2d(1, self.filter1_num, [window_size, 300], padding=((window_size - 1) // 2, 0))
            for window_size in self.window_sizes
        ])
        self.max_pool = tnn.MaxPool1d(self.window_sizes, self.window_sizes)
        self.lstm = tnn.LSTM(self.window_num * self.filter1_num, 100, num_layers=3, batch_first=True, bidirectional=True, dropout=0.5)
    ...
    def forward(self, input, length):
        x = torch.unsqueeze(input, 1)       # [B, C, T, E] Add a channel dim.
        xs = []
        for cnn in self.cnn_layers:
            x2 = tnn.functional.relu(cnn(x))        # [B, F, T, 1]
            # x2 = torch.squeeze(x2, -1)  # [B, F, T]
            # x2 = F.max_pool1d(x2, x2.size(2))  # [B, F, 1]
            xs.append(x2)
        x = torch.cat(xs, 3)            # [B, F, window]
        x = torch.transpose(x,1,2)
        x = x.reshape(x.shape[0], x.shape[1], self.window_num * self.filter1_num)
        # x = out
        # x = torch.squeeze(x, -1)
        out, _ = self.lstm(x)
        out = out[:, -1, :]
        ...
        rating = rating.view(out1.shape[0],1000)
        rating = self.rating_body2(rating)
        categary = self.catagrey_body1(out1)
        categary = categary.view(out1.shape[0],1000)
...
optimiser = toptim.Adam(net.parameters(), lr=0.005, betas=(0.9, 0.999), eps=1e-08)
